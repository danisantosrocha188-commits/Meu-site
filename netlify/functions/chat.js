exports.handler = async (event, context) => {
    ...
    if (!API_KEY) {
        return {
            statusCode: 500,
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ error: "NUCLEO OFFLINE: chave da IA ausente." })
        };
    }
    ...
    try {
        ...
        if (!response.ok) {
            return {
                statusCode: 502,
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ error: "NUCLEO OFFLINE: " + (data?.error?.message || "Erro na API.") })
            };
        }
        ...
        return {
            statusCode: 502,
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ error: "Sem resposta dos nucleos centrais." })
        };
        ...
        if (err.name === "AbortError") {
            return {
                statusCode: 504,
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ error: "TIMEOUT: nucleos demoraram demais para responder." })
            };
        }
        return {
            statusCode: 500,
